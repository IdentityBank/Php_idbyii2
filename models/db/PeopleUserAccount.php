...
/**
 * Class PeopleUserAccount
 *
 * @package idbyii2\models\db
 */
class PeopleUserAccount extends UserAccount
{
    ...
    {
        $searchModel = SearchPeopleUserAccount::instantiate
        (
            [
                'uid' => $this->uid,
                'login' => $this->login,
    ...
    public function deleteSearch()
    {
        ...
        $searchModel = SearchPeopleUserAccount::findOne(['uid_hash' => $searchModel->uid_hash]);
        if ($searchModel) {
            $searchModel->delete();
        }
    }
