...
$data = json_decode($invoice[2], true);

$price = number_format(ArrayHelper::getValue($data, 'items.value', 0) / 100, 2);
$btw =  number_format($price * 100 / 121,2);
...
">&nbsp;
    ...
    <div class="header-summary" style="
        ...
        <div class="header-summary-left" style="
            ...
            text-align: left;">
            Invoice Number: <?= $invoice[3] ?>;
        </div>
        <div class="header-summary-right" style="
            float:right;
            width: 50%;
            text-align: right;">
            Invoice Date: <?= Localization::getDateTimeInvoiceFormat(new \DateTime($invoice[5])) ?>
        </div>
    </div>
    <div class="header-service-period" style="
        text-transform: uppercase;
        padding: 30px 10px 30px 10px;">
        This invoice details service charges for the period: &lt;<?= Localization::getDateTimeInvoiceFormat(new \DateTime($invoice[5])) ?> - <?= Localization::getDateTimeInvoiceFormat(new \DateTime(ArrayHelper::getValue($data, 'endDate', ''))) ?>&gt;
    </div>
</header>
<section class="recipient">
    ...
    <div class="recipient-details" style="padding-left: 10px;">
        <p>
            <?= ArrayHelper::getValue($data, 'contactName', '&lt;Business Contact Name&gt;') ?><br/>
            <?= ArrayHelper::getValue($data, 'businessName', '&lt;Business Name&gt;') ?><br/>
            <?= ArrayHelper::getValue($data, 'address', '&lt;Address 1&gt; | &lt;Address 2&gt; | &lt;City&gt; | &lt;Postcode&gt; | &lt;Country&gt;') ?><br/>
            <?= ArrayHelper::getValue($data, 'vatNumber', '&lt;Vat Number&gt;') ?>
        </p>
    </div>
</section>
    ...
    <table width=100%>
        ...
        padding: 5px 10px 5px 10px;" class="item-quantity">1
            ...
            <td style="
                ...
                <?= ArrayHelper::getValue($data, 'items.name', 'Subscription') ?>
            </td>
            <td style="
            border:0;
            font-weight: normal;
            text-align: right;
            padding: 5px 10px 5px 10px;" class="item-unitprice">&euro; <?= $btw ?>
            </td>
            <td style="
                border:0;
                font-weight: normal;
       ...
       text-align: right;
        ...
        padding: 5px 10px 5px 10px;" colspan=3 class="items-subtotal-title">Subtotal
            ...
            font-weight: normal;
            text-align: right;
            padding: 5px 10px 5px 10px;" class="items-subtotal-total">&euro; <?= $btw ?>
            </td>
        </tr>
        <tr class="item-hr">
       ...
       text-align: right;
        ...
        padding: 5px 10px 5px 10px;" colspan=3 class="items-tax-title">BTW 21%
            ...
            text-align: right;
            padding: 5px 10px 5px 10px;
            " class="items-tax-total">&euro; <?= number_format($price - $btw, 2) ?>
            </td>
        </tr>
